...
| [ADR-021](021_zero_magic_and_explicit_defaults.md) | Zero-Magic and Explicit Defaults | Adopts hydranet ADR-003 Laws 1–2 for views-baseline: no magic default values for reproducibility/output-significant params (fail loud), no hardcoded strings/positional assumptions, single named constants for permitted defaults. Amends ADR-011 §2 (`seed` becomes declared/required). |
| [ADR-022](022_parametric_climatology_baselines.md) | Parametric Climatology Baselines | Two parametric distributional baselines (`ParametricConflictology` no-hurdle, `ParametricHurdleConflictology` hurdle) that fit a distribution to conflictology's window and sample; distribution/transform registries with per-family validation, per-sample detransform, `expm1` clamp; input seam confining pandas; the closeness-to-conflictology measurement protocol (Wasserstein/energy/C2ST + same-model null + equivalence δ). |
